...
# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 11:32:29 2019
...
# 
# From the scatter plot of the previous exercise, you saw that the points seem 
# to separate into 3 clusters. You'll now create a KMeans model to find 3 clusters, 
...
seeds_df = pd.read_csv('data/seeds_dataset.txt', sep = '\t', error_bad_lines=False, names = names)
seeds_val = seeds_df.values
...
# 
# From above, you observed from the inertia plot that 3 is a good 
# number of clusters for the grain data. In fact, the grain samples come from a
# mix of 3 different grain varieties: "Kama", "Rosa" and "Canadian". 
#  
# In this exercise, cluster the grain samples into three clusters, and compare 
# the clusters to the grain varieties using a cross-tabulation.
# =============================================================================
...
labels = model.fit_predict(seeds_val)
...
names = ['alcohol', 'malicacid', 'ash', 
         'alcalinity',
         'magnesium',
         'total_phenols',
         'flavanoids', 'non_flavanoids','proanthocyanins', 'color_intens','hue',
         'diluted', 'proline']

df = pd.read_csv('data/wine.data', names = names)
...
df = pd.DataFrame({'labels':labels, 'actual': actual_labels_if_i_had_them})
...
#You are given a NumPy array movements of daily price movements from 
#2010 to 2015 (obtained from Yahoo! Finance), where each row corresponds to a 
#company, and each column corresponds to a trading day.
#
#Some stocks are more expensive than others. To account for this, include a
# Normalizer at the beginning of your pipeline. The Normalizer will separately 
# transform each company's stock price to a relative scale before the clustering 
# begins.
#
#Note that Normalizer() is different to StandardScaler(), which you used in the 
#previous exercise. While StandardScaler() standardizes features (such as the 
#features of the fish data from the previous exercise) by removing the mean and 
#scaling to unit variance, Normalizer() rescales each sample - here, 
...
#you learned that the SciPy linkage() function performs hierarchical clustering 
#on an array of samples. Use the linkage() function to obtain a hierarchical 
#clustering of the grain samples, and use dendrogram() to visualize the result
...
mergings = linkage(seeds_val, method = 'complete')
...
#you used k-means clustering to cluster companies according to their stock price
# movements. Now, you'll perform hierarchical clustering of the companies. 
# You are given a NumPy array of price movements movements, where the rows 
# correspond to companies, and a list of the company names companies. 
#
...
#In complete linkage, the distance between clusters is the distance between the 
#furthest points of the clusters. In single linkage, the distance between clusters 
...
df = pd.read_excel('data/ESC2017_GF.xlsx', header = 1, index_col = 'Countries')

eurovn_val = df.drop(['Total', 'Rank'], axis = 1).values

ev = np.transpose(eurovn_val)

country_names = [c for c in df.columns if c not in ['Rank','Total']]
...
# Calculate the linkage: mergings
mergings = linkage(ev, method = 'single')

# Plot the dendrogram
dendrogram(mergings, labels = country_names, leaf_rotation = 90, 
leaf_font_size = 6)
plt.show()
...
# 
# In the previous exercise, you saw that the intermediate clustering of the grain 
# samples at height 6 has 3 clusters. Now, use the fcluster() function to extract 
# the cluster labels for this intermediate clustering, and compare the labels with 
# the grain varieties using a cross-tabulation.
# =============================================================================

# ------------seeds_df dataset above ---------------------------------
# Perform the necessary imports
import pandas as pd
...
# =============================================================================
# You are given a sparse array artists whose rows correspond to artists and whose 
# column correspond to users. The entries give the number of times each artist was 
...
# different artists they've listened to
# 
# =============================================================================
...
# similarity to find similar musical artists. 
